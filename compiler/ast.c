...
void ast_free_expr(Expr *expr)
{
    ...
    case EXPR_LITERAL:
        ...
        {
            if (expr->as.literal.type->kind == TYPE_STRING && expr->as.literal.value.string_value != NULL)
            {
                free((void *)expr->as.literal.value.string_value);
...
void ast_free_type_shallow(Type *type)
{
    if (type == NULL)
        return;
    ...
    {
    case TYPE_FUNCTION:
        // Free the param_types array, but NOT the individual param_types[i] or return_type (shared)
        if (type->as.function.param_types != NULL)
        {
            free(type->as.function.param_types);
            type->as.function.param_types = NULL;
        }
        // Do not free return_type
        break;

    case TYPE_ARRAY:
        // No extra shallow free needed (element_type is shared, not freed)
        break;

    default:
        // Primitives have no extras
        break;
    }
