 ...
 */
 ...
 Type *common_type(Type *a, Type *to)
 {
     ...
     for (size_t i = 0; i < sizeof(CONVERSION_RULES)/sizeof(CONVERSION_RULES[0]); i++)
     {
         ...
         {
             return ast_create_primitive_type(CONVERSION_RULES[i].promotion_result);
         }
     }
 ...
 Type *type_check_array_expression(TypeChecker *checker, ArrayExpr *expr)
 {
     ...
     return ast_create_array_type(element_type);
 }
